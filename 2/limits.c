// それぞれのデータ型の表現可能な範囲を超えると、コンパイル時に警告は表示されるがエラーにはならない。また、それぞれの結果がおかしくなる
// 警告が出たデータ型も、今回の例では、int、long long、unsigned char、unsigned shortのみで、その他のデータ型については出ていない
// このようにC言語はデータ型について、あまりコンパイラが面倒を見てくれない
// そのため、常にどれくらいの値を扱うのかを念頭に置いてプログラミングを行う必要がある
#include <stdio.h>
